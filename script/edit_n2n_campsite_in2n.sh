...
python launch.py --config configs/edit-n2n.yaml --train --gpu 0 \
    system.seg_prompt="a campsite" \
    exp_root_dir=outputs/in2n/campsite \
    trainer.max_steps=1500 \
    system.prompt_processor.prompt="Make it look like it just snowed" \
    system.dds_target_prompt_processor.prompt="Make it look like it just snowed" \
    system.dds_source_prompt_processor.prompt="a photo of a campsite" \
    ...
    data.source=dataset/in2n/campsite \
    system.gs_source=dataset/model/campsite/point_cloud/iteration_30000/point_cloud.ply  \
    system.loggers.wandb.enable=true \
    system.loggers.wandb.name="edit_n2n_campsite_lady_100_den_anchor"
...
python launch.py --config configs/edit-n2n.yaml --train --gpu 0 \
    ...
    exp_root_dir=outputs/in2n/campsite \
    trainer.max_steps=1500 \
    system.prompt_processor.prompt="Make it sunset" \
    system.dds_target_prompt_processor.prompt="Make it sunset" \
    system.dds_source_prompt_processor.prompt="a photo of a campsite" \
    system.max_densify_percent=0.01 \
    system.anchor_weight_init_g0=0.05 \
    system.anchor_weight_init=0.1 \
