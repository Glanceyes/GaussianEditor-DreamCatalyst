...
python launch.py --config configs/edit-n2n.yaml --train --gpu 0 \
    system.seg_prompt="a farm" \
    exp_root_dir=outputs/in2n/farm \
    trainer.max_steps=1500 \
    system.prompt_processor.prompt="Make it autumn" \
    system.dds_target_prompt_processor.prompt="Make it autumn" \
    system.dds_source_prompt_processor.prompt="a photo of a farm" \
    ...
    data.source=dataset/in2n/farm \
    system.gs_source=dataset/model/farm/point_cloud/iteration_30000/point_cloud.ply  \
    system.loggers.wandb.enable=true \
    system.loggers.wandb.name="edit_n2n_farm_lady_100_den_anchor"
